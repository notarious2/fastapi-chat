...
async def test_send_message_to_direct_chat_succeeds_given_valid_data(
    db_session: AsyncSession, authenticated_bob_client: AsyncClient, direct_chat: Chat, bob_user: User
):
    url = f"/chat/{direct_chat.guid}/message/"
    ...
    query = select(Message).where(and_(Message.user_id == bob_user.id, Message.chat_id == direct_chat.id))
    ...
    assert message.chat_id == direct_chat.id
