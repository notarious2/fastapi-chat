...
async def test_get_or_create_direct_chat_succeeds_direct_chat_exists(
    db_session: AsyncSession, authenticated_bob_client: AsyncClient, direct_chat: Chat, emily_user: User
):
    url = "/chat/direct/"
    payload = {"recipient_user_guid": str(emily_user.guid)}
    ...
    assert response.status_code == status.HTTP_200_OK
    assert response.json()["guid"] == str(direct_chat.guid)
